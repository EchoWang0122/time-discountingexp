...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/wangshuning/Desktop/RA/time_discounting/timediscountingexpV4(0805)v2/TD0806v2ph1ph2ph3/TD_expV4_ph1ph2ph3_lastrun.py',
...
image_horrible = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
slider_evl1 = visual.Slider(win=win, name='slider_evl1',
    startValue=None, size=(0.6, 0.05), pos=(-0.4, -0.15), units=None,
...
image_evl1 = visual.ImageStim(
    win=win,
    name='image_evl1', 
    image='img/img_horriblethreat/hopic_evl1.png', mask=None,
    ori=0.0, pos=(-0.35, 0.06), size=(0.65,0.3),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-2.0)
image_evl2 = visual.ImageStim(
    win=win,
    name='image_evl2', 
    image='img/img_horriblethreat/hopic_evl2.png', mask=None,
    ori=0.0, pos=(0.35, 0.06), size=(0.65,0.3),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-3.0)
slider_evl2 = visual.Slider(win=win, name='slider_evl2',
    startValue=None, size=(0.6, 0.05), pos=(0.4, -0.15), units=None,
    labels=(1, 2, 3, 4, 5, 6, 7), ticks=(1, 2, 3, 4, 5, 6, 7), granularity=1.0,
    style='rating', styleTweaks=(), opacity=None,
    color='LightGray', fillColor='Red', borderColor='black', colorSpace='rgb',
...
text_countdownsecond = visual.TextStim(win=win, name='text_countdownsecond',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
...
for thisTrials_horribleimg in trials_horribleimg:
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    phase1Components = [image_horrible, slider_evl1, image_evl1, image_evl2, slider_evl2, image_3button, mouse_2]
    ...
    while continueRoutine:
        ...
        # *slider_evl1* updates
        if slider_evl1.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
            # keep track of start time/frame for later
            slider_evl1.frameNStart = frameN  # exact frame index
            slider_evl1.tStart = t  # local t and not account for scr refresh
            slider_evl1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(slider_evl1, 'tStartRefresh')  # time at next scr refresh
        ...
        # *image_evl2* updates
        if image_evl2.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
            # keep track of start time/frame for later
            image_evl2.frameNStart = frameN  # exact frame index
            image_evl2.tStart = t  # local t and not account for scr refresh
            image_evl2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(image_evl2, 'tStartRefresh')  # time at next scr refresh
            image_evl2.setAutoDraw(True)
        
        # *slider_evl2* updates
        if slider_evl2.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
            # keep track of start time/frame for later
            slider_evl2.frameNStart = frameN  # exact frame index
            slider_evl2.tStart = t  # local t and not account for scr refresh
            slider_evl2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(slider_evl2, 'tStartRefresh')  # time at next scr refresh
    ...
    trials_horribleimg.addData('slider_evl1.response', slider_evl1.getRating())
    trials_horribleimg.addData('slider_evl1.rt', slider_evl1.getRT())
    trials_horribleimg.addData('slider_evl1.started', slider_evl1.tStartRefresh)
    trials_horribleimg.addData('slider_evl1.stopped', slider_evl1.tStopRefresh)
    ...
    trials_horribleimg.addData('image_evl2.started', image_evl2.tStartRefresh)
    trials_horribleimg.addData('image_evl2.stopped', image_evl2.tStopRefresh)
    trials_horribleimg.addData('slider_evl2.response', slider_evl2.getRating())
    trials_horribleimg.addData('slider_evl2.rt', slider_evl2.getRT())
    trials_horribleimg.addData('slider_evl2.started', slider_evl2.tStartRefresh)
    trials_horribleimg.addData('slider_evl2.stopped', slider_evl2.tStopRefresh)
...
_key_resp_5_allKeys = []
# keep track of which components have finished
phase2introComponents = [image_phase2intro, key_resp_5]
...
# set up handler to look after randomisation of conditions etc
trials_countdown = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('cond/countdownsecond.xlsx'),
    seed=None, name='trials_countdown')
thisExp.addLoop(trials_countdown)  # add the loop to the experiment
thisTrials_countdown = trials_countdown.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_countdown.rgb)
if thisTrials_countdown != None:
    for paramName in thisTrials_countdown:
        exec('{} = thisTrials_countdown[paramName]'.format(paramName))

for thisTrials_countdown in trials_countdown:
    currentLoop = trials_countdown
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_countdown.rgb)
    if thisTrials_countdown != None:
        for paramName in thisTrials_countdown:
            exec('{} = thisTrials_countdown[paramName]'.format(paramName))
    ...
    # keep track of which components have finished
    countdownComponents = [text_countdownsecond]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if text_countdownsecond.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_countdownsecond.frameNStart = frameN  # exact frame index
            text_countdownsecond.tStart = t  # local t and not account for scr refresh
            text_countdownsecond.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_countdownsecond, 'tStartRefresh')  # time at next scr refresh
            text_countdownsecond.setAutoDraw(True)
        if text_countdownsecond.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_countdownsecond.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                text_countdownsecond.tStop = t  # not accounting for scr refresh
                text_countdownsecond.frameNStop = frameN  # exact frame index
                win.timeOnFlip(text_countdownsecond, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in countdownComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_countdown.addData('text_countdownsecond.started', text_countdownsecond.tStartRefresh)
    trials_countdown.addData('text_countdownsecond.stopped', text_countdownsecond.tStopRefresh)
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'trials_countdown'
